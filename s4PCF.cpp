...
void usage() {
    ...
    fprintf(stderr, "   -rmin_long <rmin_long>: The minimum radius of the long side bin.  Default 60.\n");
    fprintf(stderr, "   -rmax_long <rmax_long>: The maximum radius of the long side bin.  Default 120.\n");
    fprintf(stderr, "\n");
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "One other important parameter can only be set during compilations:\n");
...
int main(int argc, char *argv[]) {
	...
	else if (!strcmp(argv[i],"-rmax_long")||!strcmp(argv[i],"-max_long")) rmax_long = atof(argv[++i]);
  else if (!strcmp(argv[i],"-rmin_long")||!strcmp(argv[i],"-min_long")) rmin_long = atof(argv[++i]);
    ...
    printf("# Pairs:    %6.3f s (%4.1f%%)\n", PairTime.Elapsed(), PairTime.Elapsed()/TotalTime.Elapsed()*100.0);

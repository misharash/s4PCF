...
void compute_pairs(Grid *grid, Float rmin, Float rmax, Float rmin_long, Float rmax_long, int np) {
    int maxsep = ceil(rmax/grid->cellsize);   // Maximum distance we must search
    int maxsep_long = ceil(rmax_long/grid->cellsize);   // Maximum distance we must search for long side
    int ne;
	      ...
	      //Acumulate powers here - code in NPCF.h (uses GPU kernels)
	        ...
	        for (delta.y = -maxsep_long; delta.y <= maxsep_long; delta.y++)
		        ...
		        // Now loop over the particles in this secondary cell
		            ...
		            Float norm2 = dx.norm2();
		            // Check if this is in the correct binning ranges
                            if (norm2<rmax_long2 && norm2>rmin_long2) cnt++; else continue;
